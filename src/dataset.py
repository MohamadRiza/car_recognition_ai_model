...
CAR_MAKE_TO_IDX   = {m: i for i, m in enumerate(CAR_MAKES)}
...
IDX_TO_CAR_MAKE   = {v: k for k, v in CAR_MAKE_TO_IDX.items()}
...
class CarDataset(Dataset):
        ...
        for class_name in sorted(os.listdir(split_dir)):
            ...
            for img_file in class_dir.iterdir():
                if img_file.suffix.lower() in [".jpg", ".jpeg", ".png", ".webp"]:
                    ...
                    # Parse attributes from filename if generated by test generator
                    # Format: car_<make>_<color>_<fuel>_<trans>_*.jpg
                    parts = img_file.stem.split("_")
                    if len(parts) >= 5 and parts[0] == "car":
                        m_cand = parts[1].title()
                        c_cand = parts[2].title()
                        f_cand = parts[3].title()
                        t_cand = parts[4].title()
                        
                        # Handle Plug-in Hybrid
                        if f_cand == "Plug" and len(parts) >= 6 and parts[4].lower() == "in" and parts[5].lower() == "hybrid":
                            fuel_val = "Plug-in Hybrid"
                            t_cand = parts[6].title() if len(parts) >= 7 else "Automatic"
                        else:
                            if f_cand in FUEL_TYPES:
                                fuel_val = f_cand
                        
                        if m_cand in CAR_MAKES:
                            make_val = m_cand
                        if c_cand in CAR_COLORS:
                            color_val = c_cand
                        if t_cand in TRANSMISSION_TYPES:
                            trans_val = t_cand
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        # Build label tensors
        labels = {
            "make":         torch.tensor(CAR_MAKE_TO_IDX.get(sample.get("make", "Toyota"), 0), dtype=torch.long),
